...
(function($) {
	...
	var methods = {
		 ...
		 * Takes an optional params: a list of options ie.
		 * jQuery("#formID1").validationEngine('attach', {promptPosition :
		 * "centerRight"});
		 */
		attach : function(userOptions) {
			...
			if (!options.binded) {
				if (options.bindMethod == "bind") {
					...
					// bind fields
					form.find("[class*=validate]").not("[type=checkbox]").not(
							"[type=radio]").not(".datepicker").bind(
					...
					form.find("[class*=validate][class*=datepicker]").bind(
							options.validationEventTrigger, {
								"delay" : 300
							}, methods._onFieldEvent);
					...
					form.find("[class*=validate]").not("[type=checkbox]").not(
							".datepicker").live(options.validationEventTrigger,
							methods._onFieldEvent);
					form.find("[class*=validate][type=checkbox]").live("click",
							methods._onFieldEvent);
					form.find("[class*=validate][class*=datepicker]").live(
							options.validationEventTrigger, {
								"delay" : 300
							}, methods._onFieldEvent);
		...
		/**
		 * Validates the form fields, shows prompts accordingly. Note: There is
		 * no ajax form validation with this method, only field ajax validation
		 * are evaluated
		 *
		...
		/**
		 * Validates one field, shows prompt accordingly. Note: There is no ajax
		 * form validation with this method, only field ajax validation are
		 * evaluated
		 *
		 * @return true if the form validates, false if it fails
		 */
		...
		/**
		 * Validates the form fields, shows prompts accordingly. Note: this
		 * methods performs fields and form ajax validations(if setup)
		 *
		 * @return true if the form validates, false if it fails, undefined if
		 *         ajax is used for form validation
		 */
		...
		updatePromptsPosition : function(event) {
			...
			form.find('[class*=validate]').not(':hidden').not(":disabled")
					.each(
							function() {
								var field = $(this);
		...
		/**
		 * Displays a prompt on a element. Note that the element needs an id!
		 *
		 * @param {String}
		 *            promptText html text to display type
		 * @param {String}
		 *            type the type of bubble: 'pass' (green), 'load' (black)
		 *            anything else (red)
		 * @param {String}
		 *            possible values topLeft, topRight, bottomLeft,
		...
		_onSubmitEvent : function() {
			...
			// validate each field (- skip field ajax validation, no necessary
			// since we will perform an ajax form validation)
		...
		_validateFields : function(form, skipAjaxValidation) {
			...
			form.find('[class*=validate]').not(':hidden').not(":disabled")
					.each(
							function() {
								var field = $(this);
								errorFound |= methods._validateField(field,
										options, skipAjaxValidation);
				...
				if (options.scroll) {
					...
					if (typeof (positionType) == 'string') {
						if (positionType.indexOf(":") != -1) {
							positionType = positionType.substring(0,
					...
					// get the position of the first error, there should be at
					// least one, no need to check this
					// var destination =
		...
		/**
		 * This method is called to perform an ajax form validation. During this
		 * process all the (field, value) pairs are sent to the server which
		 * returns a list of invalid fields or true
		 *
		 * @param {jqObject}
		 *            form
		 * @param {Map}
		...
		_validateField : function(field, options, skipAjaxValidation) {
			...
			for ( var i = 0; i < rules.length; i++) {
				...
				case "checkPhone":
					required = true;
					errorMsg = methods._checkPhone(field, rules, i, options);
					break;
				case "checkZipcode":
					required = true;
					errorMsg = methods._checkZipcode(field, rules, i, options);
					break;
				case "checkPassword":
					required = true;
					errorMsg = methods._checkPassword(field, rules, i, options);
					break;
				case "custom":
					required = true;
					...
					break;
				case "groupRequired":
					// Check is its the first of group, if not, reload
					// validation with first field
				...
				case "dateRange":
					...
					// if one entry out of the pair has value then proceed to
					// run through validation
					if (firstOfGroup[0].value || secondOfGroup[0].value) {
						errorMsg = methods._dateRange(firstOfGroup,
								secondOfGroup, rules, i, options);
					}
					if (errorMsg)
						required = true;
					options.showArrow = false;
				...
				case "dateTimeRange":
					...
					// run through validation
					if (firstOfGroup[0].value || secondOfGroup[0].value) {
						errorMsg = methods._dateTimeRange(firstOfGroup,
								secondOfGroup, rules, i, options);
					}
					if (errorMsg)
						required = true;
					...
					break;
				case "maxCheckbox":
					errorMsg = methods._maxCheckbox(form, field, rules, i,
							options);
					field = $(form.find("input[name='" + fieldName + "']"));
					break;
				case "minCheckbox":
					errorMsg = methods._minCheckbox(form, field, rules, i,
							options);
					field = $(form.find("input[name='" + fieldName + "']"));
					break;
				case "equals":
		...
		_checkPhone : function(field, rules, i, options){
			if (!field.val())
			return options.allrules[rules[i]].alertText;
		...
		_checkZipcode : function(field, rules, i, options){
			if (!field.val())
			return options.allrules[rules[i]].alertText;
			var integerRegEx = new RegExp(/^[\-\+]?\d+$/);
			if(!integerRegEx.test(field.val()))
		...
		_checkPassword : function(field, rules, i, options){
			if(!field.val())
			return options.allrules[rules[i]].alertText;
		...
		_dateRange : function(first, second, rules, i, options) {
			...
			// are not both dates
			if (!methods._isDate(first[0].value)
					|| !methods._isDate(second[0].value)) {
				return options.allrules[rules[i]].alertText
						+ options.allrules[rules[i]].alertText2;
			}
		...
		_maxCheckbox : function(form, field, rules, i, options) {
			...
			var groupSize = form
					.find("input[name='" + groupname + "']:checked").size();
						...
						.ajax({
							...
							success : function(json) {
								...
								if (errorField.length == 1) {
									...
									var msg = json[2];
									if (!status) {
										// Houston we got a problem - display an
										// red prompt
		 ...
		 *            anything else (red)
		 * @param {boolean}
		 *            ajaxed - use to mark fields than being validated with ajax
		 * @param {Map}
		...
		_buildPrompt : function(field, promptText, type, ajaxed, options) {
			...
			var promptContent = $('<div>').addClass("formErrorContent").html(
					promptText).appendTo(prompt);
			...
			if (options.showArrow) {
				...
				if (typeof (positionType) == 'string') {
					if (positionType.indexOf(":") != -1) {
						positionType = positionType.substring(0, positionType
								.indexOf(":"));
					}
					;
				}
		...
		_calculatePosition : function(field, promptElmt, options) {
			...
			// for example:
			// bottomLeft:+20 means bottomLeft position shifted by 20 pixels
			// right horizontally
			...
			if (typeof (positionType) == 'string') {
				...
				if (positionType.indexOf(":") != -1) {
					shift1 = positionType
							.substring(positionType.indexOf(":") + 1);
					positionType = positionType.substring(0, positionType
							.indexOf(":"));

					// if any advanced positioning will be needed (percents or
					// something else) - parser should be added here
		...
		_saveOptions : function(form, options) {
			...
			var userOptions = $
					.extend({}, $.validationEngine.defaults, options);
	...
	$.validationEngine = {
		defaults : {
			...
			inlineAjax : false,
			// if set to true, the form data is sent asynchronously via ajax to
			// the form.action url (get)
			ajaxFormValidation : false,
			// Ajax form validation callback method: boolean onComplete(form,
			// status, errors, options)
			...
			// Stops form from submitting and execute function assiciated with
			// it
			...
			// created.
			// the array is used during ajax form validation to detect issues
			// early and prevent an expensive submit
			ajaxValidCache : {},
