...
def request1(address,m="GET"):
        url = "http://api.map.baidu.com/geocoder/v2/?output=json"
        params = {
            "address": address,  # APP Key
            "city": "遵义市",  # JSON格式版本(0或1)默认为0
            'ak':'G5ELAErtBcu6SHUIChdzArKE'
        ...
        _logger.info("location of res is")
        _logger.info(res)
        if res:
            status = res["status"]
            if status == 0:

                return str(res['result']['location']['lng'])+","+str(res['result']['location']['lat'])
                # 成功请求
...
class SchoolSchool(models.Model):
    ...
    #     'com_name'
    #     )
    # def get_lang_long(self):
    #     #
    #     # res = {}
    #     # for lead in self.browse(cr, uid, ids, context=context):
    #     #     res[lead.id] = self.request1(lead.com_name)
    #     # return res
    #     self.school_latitude_longitude=self.request1(self.com_name)
    #     #print("123")
    #     # for record in self:
    #     #     latitude_longitude=self.request1(record.com_name)
    #     #     record.school_latitude_longitude=latitude_longitude
    #     #     record.school_latitude=str(latitude_longitude[1])
    #     #     record.school_longitude=str(latitude_longitude[0])
    ...
    #     for record in self:
    #         record.school_latitude_longitude
            # if not record.document: continue
            # with open(record.get_document_path()) as f:
            #     f.write(record.document)


    # def _get_book_amount(self, cr, uid, ids, context=None):
    #     rest=[]
    #     print "asdfasdfasdfasdfasfasdf"
    #     print ids
    #     for school in self.browse(cr, uid, ids):
    #         select_str = "select count(*) from product_product where school_id=%s" %(school.id,)
    ...
    def geo_localize(self, cr, uid, ids, context=None):
        ...
        for school in self.browse(cr, uid, ids):
            if not school:
                continue
            result = request1(school.com_name)
            if result:
                self.write(cr, uid, [school.id], {
                    'school_latitude_longitude': result,
                }, context=context)
        return True
    ...
    #
    #
    #     select_str = "select count(*) from product_product where school_id=%s" %(self.id,)
    #     _logger.info(select_str)
    #     cr.execute(select_str)
    #     res = cr.fetchone()
    ...
    # @api.one
    # @api.depends('book_amount')
    # def _get_issue_rate(self):
    #     sql = "select case when %s =0 then 0 else round(((select count(*) from library_book_issue where school_id=%s)* 100.0 /%s),2) end as issue_rate" % (self.book_amount,self.school_id,self.book_amount);
    #     self.env.cr.execute(sql)
    #     res = self.env.cr.fetchone()
    #     self.issue_rate=int(res[0])

    @api.one
    @api.depends('name')
    def _get_issue_amount(self):
        #self.issue_amount=self.issues
    ...
    # avg_book=fields.Function(_get_avg_book,string='图书增长率',)
    ...
    issue_amount=fields.Integer(compute='_get_issue_amount',store=True, string='借书总数',help="借书总数")
    student_amount = fields.Integer(compute='_get_student_amount',store=True, string='读者总数',help="读者总数")
    book_amount = fields.Integer(compute='_get_book_amount',store=True, string='书籍总数',help="书籍总数")
    division = fields.Selection([("0", '小学'),
                                 ("1", '中学'),("2", '九年一贯')],
                                '学部', help="Select division"
                                )
    #issue_rate=fields.Float(compute='_get_issue_rate',store=True, string='借阅率',help="借阅率")

    # issue_rate=fields.Float(compute='_get_issue_rate',store=True, string='借阅率',help="借阅率")

    #book_average=fields.Float(compute='_get_book_average_rate',store=True, string='生均册数',help="生均册数")
...
class SubjectSubject(models.Model):
    ...
    # book_sum=fields.Integer("图书总数",func=_get_book_sum)
    # student_sum=fields.Integer("图书总数",func=_get_student_sum)
...
class StudentStudent(models.Model):
    ...
    def createBatch(self, vals):
        ...
        '''
        for student in vals:
            if student.get('pid', False):
                sql="select count(*) as student_num from student_student where school_id=%s and pid='%s'" % (student['school_id'],student['pid'])
                _logger.info(sql)
                self.env.cr.execute(sql)
                dataa=self.env.cr.fetchone()
                _logger.info(dataa)
                _logger.info(dataa[0])
                if dataa[0]>0:
                    _logger.info("has_created")
                    continue
                student['login'] = str(student['school_id'])+'_'+str(student['pid'])
                student['password'] = student['pid']
                student['state'] = 'done'
            else:
                raise except_orm(_('Error!'), _('PID not valid, so record will \
                                             not save.'))
            result = super(StudentStudent, self).create(student)
            sqlup="update school_school set student_amount=student_amount+1 where id=%s" % student['school_id']
    ...
    def create(self, vals):
        _logger.info(vals)
        _logger.info(vals['school_id'])
        _logger.info(vals['pid'])
        if vals.get('pid', False):
            sql="select count(*) as student_num from student_student where school_id=%s and pid='%s'" % (vals['school_id'],vals['pid'])
            _logger.info(sql)
            self.env.cr.execute(sql)
            dataa=self.env.cr.fetchone()
            _logger.info(dataa[0])
            if dataa[0]>0:
                _logger.info("has_created")
                return
            vals['login'] = str(vals['school_id'])+'_'+str(vals['pid'])
    ...
    student_name = fields.Char(related='user_id.name', string='姓名',
                               store=True, readonly=True)
    pid = fields.Char('编号', required=True, default=lambda obj:
...
#    'student_id','Attachment')
    student_discipline_line = fields.One2many('student.descipline',
                                              'student_id',
                                              string='Descipline')
    address_ids = osv.fields.related('res.partner', 'student_id',string='Contacts')
    #address_ids=fields.Many2one(RE)
